...
func (suite *OrderRepositoryTestSuite) TestListOrders() {
	...
	rows, err := suite.Db.Query("SELECT * from orders")
	suite.NoError(err)
	defer rows.Close()
	...
	suite.Equal(order.ID, ordersResult[0].ID)
	suite.Equal(order.Price, ordersResult[0].Price)
	suite.Equal(order.Tax, ordersResult[0].Tax)
	suite.Equal(order.FinalPrice, ordersResult[0].FinalPrice)
}
